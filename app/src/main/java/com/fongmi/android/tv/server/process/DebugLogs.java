...
public class DebugLogs implements Process {
    ...
    public boolean isRequest(IHTTPSession session, String url) {
        return url.startsWith("/debug/logs") || url.startsWith("/debug/mpd") || url.startsWith("/debug/stream") || url.startsWith("/debug/clear") || url.startsWith("/debug/enable") || url.startsWith("/debug/disable")
                || url.startsWith("/spider/log") || url.startsWith("/spider/stream");
    }
    ...
    public Response doResponse(IHTTPSession session, String url, Map<String, String> files) {
        ...
        if (url.startsWith("/spider/stream")) return spiderStream(session);
        if (url.startsWith("/spider/log.txt")) return spiderDownload(session);
        if (url.startsWith("/spider/log")) return spiderPage(session);
        return page();
    }
    ...
    private String spiderSite(IHTTPSession session) {
        String site = session.getParms().get("site");
        return TextUtils.isEmpty(site) ? "" : site.trim();
    }

    private Response spiderDownload(IHTTPSession session) {
        String site = spiderSite(session);
        String text = DebugLogStore.text(site);
        byte[] data = text.getBytes(StandardCharsets.UTF_8);
        Response response = NanoHTTPD.newFixedLengthResponse(Response.Status.OK, "text/plain; charset=utf-8", new ByteArrayInputStream(data), data.length);
        response.addHeader("Content-Disposition", "attachment; filename=webhtv-spider-log.txt");
        response.addHeader("X-Content-Type-Options", "nosniff");
        return noCache(response, null);
    }

    private Response spiderStream(IHTTPSession session) {
        String site = spiderSite(session);
        long version = DebugLogStore.version();
        boolean unchanged = version == paramLong(session, "v", -1);
        String text = unchanged ? null : DebugLogStore.text(site);
        String body = "{\"enabled\":" + DebugLogStore.isEnabled() + ",\"site\":\"" + json(site) + "\",\"size\":" + DebugLogStore.size(site) + ",\"version\":" + version + ",\"text\":" + (unchanged ? "null" : "\"" + json(text) + "\"") + "}";
        return noCache(NanoHTTPD.newFixedLengthResponse(Response.Status.OK, "application/json; charset=utf-8", body), null);
    }

    private Response spiderPage(IHTTPSession session) {
        String site = spiderSite(session);
        String siteParam = escape(site);
        String logs = escape(DebugLogStore.text(site));
        String html = "<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Spider 调试日志</title>"
                + "<style>body{margin:0;background:#f4f6f8;color:#202124;font:14px sans-serif}main{max-width:1200px;margin:auto;padding:10px}.bar{display:flex;gap:8px;flex-wrap:wrap;align-items:center;background:#fff;padding:10px;border-radius:8px;border:1px solid #ddd}h2{margin:0;font-size:17px}input,button{padding:7px 9px;border:1px solid #ccc;border-radius:6px;background:#fff;font:inherit}input{flex:1;min-width:180px}.meta{color:#666;font-size:12px}pre{white-space:pre-wrap;word-break:break-all;background:#fff;border:1px solid #ddd;border-radius:8px;padding:10px;line-height:1.5;min-height:60vh}</style></head><body><main>"
                + "<div class=\"bar\"><h2>Spider 调试日志</h2><input id=\"site\" value=\"" + siteParam + "\" placeholder=\"site key，例如 csp_xxx\"><button onclick=\"go()\">过滤</button><a href=\"/spider/log.txt?site=" + siteParam + "\">下载</a><span id=\"meta\" class=\"meta\"></span></div>"
                + "<p>用法：加载 Jar 后，在 WebHomeTV 中执行搜索/分类/详情/播放，再打开 <b>/spider/log?site=站源key</b>。日志只显示该 Spider 调用期间标记的内容。</p>"
                + "<pre id=\"logs\">" + logs + "</pre>"
                + "<script>let v=-1;function go(){const s=document.getElementById('site').value.trim();location.href='/spider/log?site='+encodeURIComponent(s)}async function poll(){try{const s=document.getElementById('site').value.trim();const r=await fetch('/spider/stream?site='+encodeURIComponent(s)+'&v='+v+'&_='+Date.now(),{cache:'no-store'});const j=await r.json();v=j.version||v;document.getElementById('meta').textContent=(j.enabled?'开启':'关闭')+' · '+j.size+' 行';if(j.text!==null){document.getElementById('logs').textContent=j.text||'暂无该 Spider 的调试日志';}}catch(e){}setTimeout(poll,1000)}poll();</script></main></body></html>";

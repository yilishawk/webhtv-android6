...
| FFmpeg renderer | `io.github.anilbeesetti:nextlib-media3ext:1.10.0-0.12.1-fongmi-softload-av3a-ffmpeg901-r1`；内置 FongMi FFmpeg 9.0.1（`177f090e0503b7e013922ca903bde14b1c375f18`）与 libarcdav3a |
...
Gradle Wrapper 和仓库脚本使用 `JAVA_HOME`/`PATH` 中的 JDK，以及 `ANDROID_HOME`、`ANDROID_SDK_ROOT` 或 `local.properties` 指定的 Android SDK。
...
JDK 应独立安装并通过标准的 `JAVA_HOME`/`PATH` 配置。不要在项目脚本或共享文档中写死某台机器上的 IDE/JBR 路径；`java -version` 必须显示 21。
...
命令行构建需要根目录有 `local.properties`。它是普通文本配置，在 clone 后进入仓库根目录直接生成：
...
如 SDK 未安装 API 37，使用 Android SDK 自带的 `sdkmanager` 执行：
...
`platforms;android-37.0` 是当前 `sdkmanager` 返回的实际包名；Gradle 中仍写 `compileSdk=37`。如果 `cmdline-tools/latest/bin/sdkmanager` 不存在，请安装官方 Android SDK Command-line Tools，并将其按 `cmdline-tools/latest/` 目录结构放入 SDK。
...
- `third_party/patches/nextlib-av3a.patch`：把 nextlib FFmpeg 升级到 FongMi FFmpeg 9.0.1（`177f090e0503b7e013922ca903bde14b1c375f18`），编译并静态链接 `dependency/avs3a`，映射 `audio/av3a -> libarcdav3a`，支持 10 声道 planar PCM 和必要时立体声下混。
...
当前 `libmpv.so` 已启用 OpenGL、Vulkan、MediaCodec AImageReader/AHardwareBuffer、libcurl 和 HTTP/2。截至 2026-08-29，两套 ABI assets 已按当前 lock、构建脚本和补丁集通过能力标记与 ELF 校验：
...
| FFmpeg | `177f090e0503b7e013922ca903bde14b1c375f18`（9.0.1-fongmi，C0-M 已验证） |
...
| `.../cmdline-tools/latest/bin/sdkmanager: No such file or directory` | 安装官方 Android SDK Command-line Tools，确认其位于 `$ANDROID_HOME/cmdline-tools/latest/`，并检查 `ANDROID_HOME` |
